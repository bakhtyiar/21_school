...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/09 15:12:16 by gnameles          #+#    #+#             */
/*   Updated: 2021/11/09 15:12:20 by gnameles         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*line;
	static char	*l_stash[8192];

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	l_stash[fd] = ft_read_to_l_stash(fd, l_stash[fd]);
	if (!l_stash[fd])
		return (NULL);
	line = ft_get_line(l_stash[fd]);
	l_stash[fd] = ft_new_l_stash(l_stash[fd]);
	return (line);
}
